...
function Content() {
	...
	const currentContentLength = useMemo(() => articleContent.length, [articleContent]);
		...
		<CreatePageWrapper>
			...
			<Forms>
				...
				<Form>
					...
					/>
					<Textarea value={articleContent} onChange={handleChangeArticleContent} />
				</Form>
				<LinkForm>
					<LabelBox label={'You can add one link or click on button to add more'} />
					<AddLinkButton variant='outlined' onClick={handleAddLink}>
						Add Link
					</AddLinkButton>
					...
					{links.map((link) => (
						<LinkBox key={link.id}>
							<LinkField
								...
								value={link.url}
								onChange={(e: any) => handleChangeLink(e.target.value, link.id)}
							/>
							<DeleteOutlinedIcon onClick={() => handleDeleteLink(link.id)} />
						</LinkBox>
					))}
				</LinkForm>
			</Forms>
