...
import { createCathedraRequest, getCathedrasRequest, deleteCathedraRequest } from '../api/cathedra';
import { CreateCathedraType } from '../types/cathedra';

function* createCathedra(action: { type: string; payload: CreateCathedraType }) {
	try {
		// @ts-ignore
